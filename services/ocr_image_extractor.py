...
def generate_ocr_for_all_images():
    ...
    try:
        images_with_ocr = db.session.query(OCRResults.image_id).distinct().subquery()
        images = Images.query.filter(Images.id.notin_(images_with_ocr)).all()
    except Exception as e:
...
def extract_ocr_for_images(image_ids):
    """Generate OCR text for specific images identified by their IDs"""
    try:
    ...
    logger.info(f"OCR processing for specific images complete. Success: {processed}, Errors: {errors}, Total: {total}")
    return results
